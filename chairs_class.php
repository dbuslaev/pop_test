...
class ChairsCircle {
  ...
  var $chairs_total;
  var $chairs_skip_increment;
  var $chairs_sequence;

  function ChairsCircle($chairs_total = 100, $chairs_skip_increment = 1){
    $this->chairs_total = $chairs_total;
    $this->chairs_skip_increment = $chairs_skip_increment;
  }

  function getSurvivingChairNumber(){
    $chairs_sequence = $this->getSurvivingSequence();
    return array_pop($chairs_sequence);
  }

  // creates sequence only the first time and saves it for later
  function getSurvivingSequence(){
    if(!is_array($this->chairs_sequence)){
      $this->chairs_sequence = $this->createSurvivingSequence();
    }
    return $this->chairs_sequence;
  }

  private function createSurvivingSequence(){
    $chairs_total = $this->chairs_total;
    $chairs_skip_increment = $this->chairs_skip_increment;
...
$circle = new ChairsCircle(100, 1);
$survivor = $circle->getSurvivingChairNumber();
$chairs_sequence = $circle->getSurvivingSequence();
